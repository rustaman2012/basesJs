...
(function (arrOfTasks) {
  ...
  // START Цвета елементов для темы
  const themes = {
    default: {
  ...
  function onFormSubmitHandler(e) {
    ...
    if (!titleValue || !bodyValue) {
      alert("Заполните все елементы формы!");
      return;
    }
  ...
  function createNewTask(title, body) {
    const newTask = {
      ...
      body,
      completed: false,
      _id: `task-${Math.round(Math.random() * 1000000000000000)}`,
    };
  ...
  function deleteTaskFomHtml(confirmed, el) {
    if (!confirmed) return;
    el.remove();
  }
  ...
  function onDeleteHandler({ target }) {
    if (target.classList.contains("delete-btn")) {
      ...
      deleteTaskFomHtml(confirmed, parent);
    }
  }
  ...
  function setTheme(name) {
    const selectetThemeObj = themes[name];
