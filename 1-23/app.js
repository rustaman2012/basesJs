...
const arr3 = [[14, 45], [1], ["a", "c", "d"]];

arr3.sort((prev, next) => prev.length - next.length);
...
coreArr.sort((prev, next) => prev.info.cores - next.info.cores);
...
function filterProduct(arr, min, max) {
  const products = arr.filter(
    (price) => price.price >= min && price.price <= max
  );
  return products.sort((prev, next) => prev.price - next.price);
}

// console.log(filterProduct(products, 15, 30));
